...
function searchUser_bUsername($db, $username){
	if(!empty($username) && !mysqli_error($db)){
		$sql = mysqli_query($db, "select id,firstName,lastName,user_email,profilePicture from user where username = '$username'");
...
function searchUser_bId($db, $userid){
	if(!empty($userid) && !mysqli_error($db)){
		$sql = mysqli_query($db, "select username,firstName,lastName,user_email,profilePicture from user where id=$userid");
		if(mysqli_num_rows($sql) > 0){
			$result = mysqli_fetch_array($sql);
